...
class ArrayContext:
    ...
    def from_array(cls, arr: Any) -> 'ArrayContext':
        ...
        if fw == Framework.TORCH:
            ...
            if hasattr(arr, 'devices') and arr.devices():
                platform = arr.devices()[0].platform.lower()
                if platform == 'gpu': 
                    device_type = DeviceType.GPU
...
def abs(x: Any) -> Any:
    ...
    if ctx.framework == Framework.TORCH:
        import torch
        return torch.abs(x)
    elif ctx.framework == Framework.TENSORFLOW:
        import tensorflow as tf
        return tf.math.abs(x)
    elif ctx.framework == Framework.JAX:
        import jax.numpy as jnp
        return jnp.abs(x)
    elif ctx.framework == Framework.CUPY:
        import cupy as cp
        return cp.abs(x)
    else:
        import numpy as np
...
def arange(end: int, ctx: ArrayContext) -> Any:
    """Генерация массива с использованием явно переданного контекста."""
    if ctx.framework == Framework.TORCH:
        import torch
        # Конвертируем Enum Device обратно в строку, понятную PyTorch
        dev_str = "cuda" if ctx.device == DeviceType.GPU else ctx.device.value
        return torch.arange(end, dtype=ctx.dtype, device=dev_str)
        
    elif ctx.framework == Framework.TENSORFLOW:
        import tensorflow as tf
        with tf.device(f'/{ctx.device.value.upper()}:0'):
            return tf.range(end, dtype=ctx.dtype)
            
    elif ctx.framework == Framework.JAX:
        import jax.numpy as jnp
        return jnp.arange(end, dtype=ctx.dtype)
        
    elif ctx.framework == Framework.CUPY:
        import cupy as cp
        return cp.arange(end, dtype=ctx.dtype)
        
    else:
        import numpy as np
        return np.arange(end, dtype=ctx.dtype)
