...
# =====================================================================
# 1. МОДУЛИ ЭХА И ФУНКЦИЯ ПОТЕРЬ (Остаются без изменений)
# =====================================================================
...
def build_causal_echo_window(derivatives_spec: SpecFunc) -> SpecFunc:
    ...
    echo_matrix = echo_matrix / (echo_matrix.sum(dim=-1, keepdim=True) + 1e-8) # Нормировка
...
# =====================================================================
# 2. ОБУЧАЮЩИЙ СКРИПТ (СЛОЖНЫЙ ТЕСТ)
# =====================================================================
if __name__ == "__main__":
    ...
    # А. Нисходящий чирп (плавное изменение частоты)
    for t in range(40, 120):
        f = int(55 - 30 * ((t - 40) / 80)) # от 55 до 25 бина
        S_true_vals[0, f-1:f+2, t] = 3.0
        
    # Б. Широкополосные щелчки (много частот в один момент времени)
    S_true_vals[0, 15:60, 180:182] = 4.0 # Щелчок 1
    S_true_vals[0, 20:50, 210:212] = 3.5 # Щелчок 2
    
    # В. Устойчивый тон (константная частота)
    ...
    # --- 2. ГЕНЕРАЦИЯ ЗАШУМЛЕННОГО ТАРГЕТА ---
    Target_clean = convolveTime(S_true, E_true).values[0].detach()
    
    # Добавляем Гауссовский шум. Амплитуда шума (0.1) достаточно заметна
    noise = torch.randn_like(Target_clean) * 0.001
    # Отрезаем отрицательные значения, спектрограмма физически >= 0
    Target_tensor = F.relu(Target_clean + noise)

    # --- 3. ИНИЦИАЛИЗАЦИЯ (Сеть ничего не знает о шуме и сигнале) ---
    ...
    P_learn_vals[..., Z_learn:] = -3.0 # Одинаковое стартовое затухание
    ...
    EPOCHS = 1000 # Дадим чуть больше времени на распутывание сложного сигнала
    # L1 немного усилена, чтобы сеть активно "чистила" фон от добавленного нами шума
    LAMBDA_L1 = 5e-2 
    loss_history = []

    print("Обучение (Сложный сигнал + Шумный Target + Combined Loss)...")
    for epoch in range(EPOCHS):
        ...
        # Обучаемся по ЗАШУМЛЕННОМУ таргету
        ...
        if (epoch + 1) % 50 == 0:
            print(f"Epoch {epoch+1:04d}/{EPOCHS} | MSE+Log: {loss_recon.item():.5f} | L1: {loss_l1.item():.5f}")
    ...
    # Фиксируем цветовую шкалу по чистому оригиналу, чтобы видеть реальные амплитуды
    vmax_S = S_true_np.max()
    vmax_E = E_true_np.max()
    vmax_T = Target_clean[0].numpy().max()

    axs[0, 0].imshow(S_true_np, aspect='auto', origin='lower', extent=extent_S, cmap='inferno', vmin=0, vmax=vmax_S); axs[0, 0].set_title("Ground Truth S (Сложный)")
    ...
    axs[0, 2].imshow(Target_np, aspect='auto', origin='lower', extent=extent_S, cmap='viridis', vmin=0, vmax=vmax_T); axs[0, 2].set_title("NOISY Target (С чем работает сеть)")
    ...
    # 41-й бин - это центральная частота горизонтального тона
    f_idx_high = 41 
    ...
    axs[2, 1].set_title(f"Производные (СЧ тон, bin={f_idx_high})"); axs[2, 1].axvline(0, color='gray', linestyle='--'); axs[2, 1].legend(); axs[2, 1].grid(True)
    
    # 25-й бин - пересекается чирпом и щелчками
    f_idx_low = 25
    ...
    axs[2, 2].set_title(f"Производные (НЧ чирп/щелчки, bin={f_idx_low})"); axs[2, 2].axvline(0, color='gray', linestyle='--'); axs[2, 2].legend(); axs[2, 2].grid(True)
