...
class BaseAdapter(Generic[T]):
    """Базовый класс для сериализации/десериализации сложных типов для QSettings"""
    def dump(self, value: T) -> Any: ...
    def load(self, raw_value: Any) -> T: ...


class EnumAdapter(BaseAdapter[Enum]):
    """Сохраняет Enum как строку, восстанавливает обратно. Защита от битых данных."""
    ...
    def load(self, raw_value: Any) -> Enum:
        try:
            return self.enum_cls[str(raw_value)]
        except (KeyError, ValueError):
            return self.fallback


class JsonAdapter(BaseAdapter[Dict[Any, Any] | List[Any]]):
    """Позволяет хранить сложные словари и списки внутри QSettings как JSON строки"""
...
class LocalesAdapter(BaseAdapter[list[QtCore.QLocale]]):
    """Тот самый обработчик списка локалей"""
...
class QtObjAdapter(BaseAdapter[T]):
    """Для QColor, QFont, QByteArray и других нативных типов Qt, которые QSettings поддерживает"""
    def __init__(self, obj_type: Type[T], fallback: T):
        self.obj_type = obj_type
        self.fallback = fallback

    def dump(self, value: T) -> Any:
        return value  # QSettings сам разберется с QColor/QFont

    def load(self, raw_value: Any) -> T:
        if isinstance(raw_value, self.obj_type):
            return raw_value
        return self.fallback
    

class QColorAdapter(BaseAdapter[QtGui.QColor]):
    """Сохраняет QColor в виде понятной HEX-строки (#RRGGBB)"""
    def __init__(self, fallback: QtGui.QColor):
        self.fallback = fallback

    def dump(self, value: QtGui.QColor) -> str:
        # Превращаем QColor в строку "#00ff00"
        return value.name() 

    def load(self, raw_value: Any) -> QtGui.QColor:
        # Превращаем строку "#00ff00" обратно в QColor
...
class SettingField[T]:
    def __init__(
        self, 
        default: T, 
        val_type: Type[T] | None = None, 
        key: str | None = None,
        signal_ref: Any = None,
        adapter: BaseAdapter[T] | None = None  # <--- Новый мощный параметр
    ) -> None:
    ...
    def __get__(self, instance: Settings | None, owner: Type[Any]) -> T | 'SettingField[T]':
        ...
        # Если значения нет в конфиге
        if raw_val is None:
            return self.default

        # 1. Если есть адаптер — пропускаем через него
        if self.adapter:
            return self.adapter.load(raw_val)

        # 2. Если есть базовый тип — просим QSettings кастануть
        if self.val_type is not None:
            return settings.value(self.key, self.default, type=self.val_type) # type: ignore
    ...
    def __set__(self, instance: Settings, value: T) -> None:
        ...
        # Сравнение (для списков/словарей/объектов Qt)
...
def Field[T](
    ...
    sig_type: Type[Any] | None = None
) -> Tuple[SettingField[T], Any]:
    
    # Автоопределение типа сигнала
    if sig_type is None:
        sig_type = val_type if val_type is not None else type(default)
        if sig_type is list or sig_type is dict: 
            sig_type = object # Qt сигналы не любят list/dict напрямую без type hinting
