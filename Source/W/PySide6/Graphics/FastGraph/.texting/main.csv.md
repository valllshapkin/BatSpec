Чтобы не загромождать тесты и сделать функционал редактирования элементов переиспользуемым, я добавил класс `FastGraphEditor` прямо в конец твоего `__init__.py`. Этот класс берет на себя всю логику по извлечению `FastElement`, скрытию его на слое, созданию правильного `pg.ROI` в зависимости от типа слоя, а также по сохранению отредактированных координат обратно в слой с правильным обновлением бакетов.

Ниже представлены два файла: обновленный `__init__.py` и новый тест `2.py`.

``````text path="/MainData/Repo/valllshapkin/MonoBat/BatSpec/Source/W/PySide6/Graphics/FastGraph/__init__.py" encoding="utf-8"
import numpy as np
...
class AbstractFastLayer(pg.GraphicsObject):  # type: ignore[misc]
    ...
    def _add_to_buckets(self, el: FastElement) -> None:
    ...
    def _remove_from_buckets(self, el: FastElement) -> None:
        x_min, x_max = self._get_x_bounds(el)
        b0: int = int(x_min // self.bucket_width)
        b1: int = int(x_max // self.bucket_width)
        for b in range(b0, b1 + 1):
...
class FastGraphEditor:
    ...
    def edit_element(self, layer: AbstractFastLayer, el_id: str) -> None:
        ...
        pen_c, brush_c = layer.group_colors.get(el.group_id, (QtGui.QColor(255, 255, 255), None))
        pen: QtGui.QPen = pg.mkPen(pen_c, width=2)  # type: ignore[no-untyped-call]
    ...
    def commit_element(self, el_id: str) -> None:
        ...
        layer._remove_from_buckets(el)
        
        # 2. Обновляем координаты элемента из ROI
        if isinstance(layer, FastRectLayer):
            pos = roi.pos()  # type: ignore[reportUnknownMemberType]
            size = roi.size()  # type: ignore[reportUnknownMemberType]
...
``````py path="/MainData/Repo/valllshapkin/MonoBat/BatSpec/Source/W/PySide6/Graphics/FastGraph/__test__/2.py" encoding="utf-8"
import sys
import random
